...
function makeSpamLimiter({ windowMs, max, reason, threshold = 3, keyFn }) {
    return rateLimit({
        ...
        standardHeaders: true,
        legacyHeaders: false,
        keyGenerator: keyFn || ((req) => req.ip || req.headers['x-forwarded-for']?.split(',')[0]?.trim() || 'unknown'),
        skip: (req) => process.env.NODE_ENV === 'test',
        handler: async (req, res) => {
            const ip = req.ip || req.headers['x-forwarded-for']?.split(',')[0]?.trim() || 'unknown';
            // pool bu noktada global olarak tanımlı olduğundan doğrudan kullanılabilir
            if (typeof pool !== 'undefined') {
                await recordSpamViolation(ip, reason, pool, threshold).catch(() => {});
            }
